...
async function replaceVars (path) {
    ...
    console.log(
      "🛠 newPublicRuntimeConfig 🛠",
      newPublicRuntimeConfig,
      "🛠 End of newPublicRuntimeConfig 🛠"
    );
    ...
    console.log(
      "🔐 newServerRuntimeConfig 🔐",
      newServerRuntimeConfig,
      "🔐 End of newServerRuntimeConfig 🔐"
    );
    const newServer = serverContent
      .replace(
        publicRegexp,
        `"publicRuntimeConfig":${JSON.stringify(newPublicRuntimeConfig)}`
      )
      .replace(
        serverRegexp,
        `"serverRuntimeConfig":${JSON.stringify(newServerRuntimeConfig)}`
      );
